...
const GameRecap = () => {
  ...
  // 当访�?/recap/latest 时，优先从ESPN拉取最近一场已结束比赛；无则尝试今日比赛；再无则回退到字�?
  useEffect(() => {
  ...
  // �?slug 动态加载差异化 recap 内容（若存在则优先渲染）
  const dynamicRecap = (() => {
    try {
      if (!gameId) return null;
      // 使用绝对路径并遍历键，兼�?Vite �?import.meta.glob 键名形式（通常�?/src/... 开头）
      ...
      if (arr.length > 0 && typeof arr[0] === 'object' && arr.every(x => typeof x === 'object')) {
        ...
        const cols = allCols
          .filter(c => {
            ...
            return true; // 保留非语言后缀列，�?time、score �?
          })
          .sort((a, b) => {
            if (a === 'time') return -1;
            if (b === 'time') return 1;
  ...
  // 选择语言文本的辅助：优先使用 {key_en}/{key_zh}，否则回退�?{key}
